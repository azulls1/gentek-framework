...
import { ConfigManager, StateManager, loadFlowDefinition, logger } from '@gentek/core';
...
export async function runStatus(opts: StatusOptions): Promise<void> {
  ...
  logger.header(`\n📊 Gentek status — ${config.projectName}\n`);
  ...
  if (state.currentPhase && !state.completedPhases.includes(state.currentPhase)) {
    ...
    logger.info(`  npx @gentek/cli cycle  (retoma la fase en curso)`);
    ...
    logger.info(kleur.bold('Próximo paso:'));
    logger.info(`  npx @gentek/cli cycle`);
  }
}
